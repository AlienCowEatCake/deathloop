...
OUT_PATH="."
ENTITLEMENTS_PATH="src/Deathloop/resources/platform/macosx/${PROJECT}.entitlements"
...
cd "${BUILDDIR}"
BUILD_PATH="${PWD}"
${CMD_QMAKE} -r CONFIG+="release" LIBS+=-dead_strip QMAKE_MAC_SDK=${MAC_SDK} QMAKE_MACOSX_DEPLOYMENT_TARGET=10.10 QMAKE_APPLE_DEVICE_ARCHS="x86_64 arm64" "../${PROJECT}.pro"
make -j3
cd "${OUT_PATH}"
