...
#include "ui_gframe_hight.h"
#include "main.h"

gframe_hight::gframe_hight(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::gframe_hight)
{
    ui->setupUi(this);
    setCentralWidget(ui->widget);   
    ui->widget->axisX = trUtf8("t, c");
    ui->widget->axisY = trUtf8("y, м");
    update();
}

gframe_hight::~gframe_hight()
{    
    delete ui;   
}

void gframe_hight::update()
{
    clear();
    iter = maction;
    ...
    while(iter.ff == 0)
    {
        ...
        if(!skip_flag)
        {
            ui->widget->masY.push_back((float)iter.y);
